...
use crate::ocpp::{
    ChargePointState, RequestToSend,
    builders::{
...
pub struct OCPPCentralSystem<'a> {
    ...
    fn visit_request_message(
        ...
        let response = match request.message_type {
            ...
            MessageTypeName::StatusNotification => {
                ...
                >(request.json)?;
                serde_json::to_value(&handle_status_notification_request(&status_notification)?)?
            }
