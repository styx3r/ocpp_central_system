...
fn repeated_pv_smart_charging_with_pv_overproduction() -> Result<(), Box<dyn Error>> {
    ...
    assert_eq!(
        integration_test.get_stored_meter_readings()?,
        vec![
            (
                "CurrentOffered".to_string(),
                16.0,
                "A".to_string(),
                "".to_string()
            ),
            (
                "PowerActiveImport".to_string(),
                3700.0,
                "W".to_string(),
                "L1".to_string()
            ),
    ...
    assert_eq!(
        integration_test.get_stored_status_notifications()?,
        vec!["Available", "Charging", "Available", "Available"]
    );
